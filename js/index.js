            ...
            <div id="collapse${collapseNum[i]}" class="accordion-collapse collapse" data-bs-parent="#accordionExample">
              <div class="accordion-body row">
                    <div class="col-md-8 ps-3 pt-3">
                        <p class="text-capitalize fs-5 mt-2">id: <span class="font-bold">${objectArr[i].id}</span></p>
                        <p class="text-capitalize fs-5 mt-2">name: <span class="font-bold">${objectArr[i].name}</span></p>
                        <p class="text-capitalize fs-5 mt-2">total of transactions: <span class="font-bold">${getTotal(objectArr[i].id)[0]}</span></p>
                        <p class="text-capitalize fs-5 mt-2">numbers of transactions: <span class="font-bold">${getTotal(objectArr[i].id)[1]}</span></p>
                    </div>
                    <div class="col-md-4 d-flex align-items-center justify-content-center">
                        <button data-id="${objectArr[i].id}" class="view btn btn-outline-primary">
    ...
    search.addEventListener(`click`, function() {
      ...
      if(isNaN(inputValue)) {
        for(let i = 0 ; i < details.customers.length;i++) {
          ...
          if(details.customers[i].name.toLowerCase().includes(inputValue.toLowerCase())) {
...
for (let i = 0; i < details.customers.length;i++) {
  if(details.customers[i].id == id) {
    addCustomerDetails = `<div class=" ps-3 pt-3">
...
document.querySelector(`.detailsContainer`).innerHTML = addCustomerDetails;
